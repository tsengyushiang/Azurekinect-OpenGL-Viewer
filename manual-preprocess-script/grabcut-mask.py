...
class InputJson:
    ...
    def depthVisualize(self):
        return np.array(self.data["depthmap_raw"]).reshape((self.height,self.width))*self.data["depthscale"]/5.0*255
...
def grabcut_batch_test(input,headSeg,outputdir,outputvideo):
    ...
    saveImg('contour_mask.png', cv2.merge((ones, ones, zeros, input.contour)))

    additional_mask_color = blendMask(headSeg,input.color)

    # cull mask w/ floor/far obj info
    headSeg[input.cullingMask==0]=0
    descriptLabel = np.ones((input.height,input.width),np.uint8) * cv2.GC_BGD
    descriptLabel[headSeg>0] = cv2.GC_PR_FGD

    # set culled mask as RR_BGD instead of BGD
    # descriptLabel[(headSeg>0) * (input.cullingMask==0)] = cv2.GC_PR_BGD
    descriptLabel[input.biggestConnectedDepth>0] = cv2.GC_FGD
    descriptLabel[input.contour>0] = cv2.GC_PR_FGD

    label_FGD = np.copy(zeros)
    label_FGD[descriptLabel==cv2.GC_FGD] += 128
    label_PRFGD = np.copy(zeros)
    label_PRFGD[descriptLabel==cv2.GC_PR_FGD] += 128
    label_PRFGD[descriptLabel==cv2.GC_PR_BGD] += 128
    label_BGD = np.copy(zeros)
    label_BGD[descriptLabel==cv2.GC_BGD] += 128
    label_BGD[descriptLabel==cv2.GC_PR_BGD] += 128

    saveImg('descriptLabel.png',cv2.merge((label_FGD, label_PRFGD, label_BGD,np.ones((input.height,input.width),np.uint8)*64)))

    bgdModel = np.zeros((1,65),np.float64)
    fgdModel = np.zeros((1,65),np.float64)
    resultLabel, bgdModel, fgdModel = cv2.grabCut(input.color,descriptLabel,None,bgdModel,fgdModel,5,cv2.GC_INIT_WITH_MASK)
    resultMask = np.where((resultLabel==cv2.GC_PR_BGD)|(resultLabel==cv2.GC_BGD),0,1).astype('uint8')

    saveImg('grabcut_mask.png', cv2.merge((ones, ones, ones, 255-resultMask*255)))

    return np.hstack((
        np.vstack((additional_mask_color,input.color)),
        np.vstack((blendMask(headSeg,input.color),blendMask(resultMask,input.color)))
    ))
...
dataExt = ".json"
getExtFilesInfolder(dataDict,"../app/20210820-146frames-testing-data",dataExt,[dataExt])
...
# outputdir = "outputCDCL-humanpart20210820"
# getExtFilesInfolder(dataDict,"../app/20210820_newfileformat_test/CDCL-humanpartSeg-output",maskExt,[maskExt,"seg_"])
...
for key in dataDict:
    ...
    videoframe = grabcut_batch_test(input,segmask,outputfolder,out)
