...
public class unityLoadMeshStructor : MonoBehaviour
{
    ...
    void OnGUI()
    {
        if(GUI.Button(new Rect(10, 10, 100, 100), String.Format("Toggle\n Project\nTexture\n {0}",useProject))){
            if(useProject==1){
                useProject=0;
            }else{
                useProject=1;
        ...
        {
            if(GUI.Button(new Rect(10, 110, 100, 100), String.Format("Load\n Next {0}",currentIndex+1))){
                setObj(++currentIndex);
            }
        }

        if ((currentIndex-1 >= 0)){
            if(GUI.Button(new Rect(10, 210, 100, 100),  String.Format("Load\n Previous {0}",currentIndex-1))){
                setObj(--currentIndex);
            }
        }
    }
